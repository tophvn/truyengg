...
// Truy vấn lấy danh sách truyện với lượt xem tuần
$query = "
    SELECT 
        c.id, 
        c.name, 
        ...
        c.updated_at, 
        c.is_hot,
        COALESCE(SUM(vl.views_increment), 0) as week_views,
    ...
    LEFT JOIN comic_view_logs vl ON c.id = vl.comic_id 
        AND vl.log_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
        AND vl.log_date <= CURDATE()
    $where
    GROUP BY c.id
    ORDER BY week_views DESC, c.updated_at DESC
    LIMIT ?, ?";
...
<body class="dark-style">
    <div class="container container-background">
        <section>
            <div class="d-flex mb15"><h1 class="title_cate mr-auto">Top Tuần</h1></div>
                    ...
                    <div class="col-md-10 col-9">
                        <div class="d-flex">
                            <a class="btn_short <?php echo $status_filter === '0' ? 'active' : ''; ?>" 
                               href="<?php echo $base_url; ?>/top-tuan.php?page=1&status=0<?php echo $country_filter ? '&country=' . $country_filter : ''; ?>">
                                Đang tiến hành
                            </a>
                            <a class="btn_short <?php echo $status_filter === '2' ? 'active' : ''; ?>" 
                               href="<?php echo $base_url; ?>/top-tuan.php?page=1&status=2<?php echo $country_filter ? '&country=' . $country_filter : ''; ?>">
                                Hoàn thành
                            </a>
                            <a class="btn_short <?php echo $status_filter === null ? 'active' : ''; ?>" 
                               href="<?php echo $base_url; ?>/top-tuan.php?page=1<?php echo $country_filter ? '&country=' . $country_filter : ''; ?>">
                                Tất cả
                            </a>
                        </div>
                    </div>
                        ...
                        <div class="d-flex">
                            ...
                            foreach ($countries as $id => $name) {
                                echo '<a class="btn_short ' . ($country_filter === $id ? 'active' : '') . '" 
                                        title="' . $name . '" 
                                        href="' . $base_url . '/top-tuan.php?page=1' . ($status_filter !== null ? '&status=' . $status_filter : '') . '&country=' . $id . '">
                                        ' . $name . '
                                      </a>';
                            }
                            ?>
                            <a class="btn_short <?php echo $country_filter === null ? 'active' : ''; ?>" 
                               href="<?php echo $base_url; ?>/top-tuan.php?page=1<?php echo $status_filter !== null ? '&status=' . $status_filter : ''; ?>">
                                Tất cả
                            </a>
                        </div>
                    </div>
            ...
            <div class="pagination mt-4 mb20">
                ...
                if ($page > 1) {
                    echo '<a class="page-item" 
                           href="' . $base_url . '/top-tuan.php?page=1' . ($status_filter !== null ? '&status=' . $status_filter : '') . ($country_filter ? '&country=' . $country_filter : '') . '">
                           <span aria-hidden="true">«</span></a>';
                    echo '<a class="page-item" 
                           href="' . $base_url . '/top-tuan.php?page=' . ($page - 1) . ($status_filter !== null ? '&status=' . $status_filter : '') . ($country_filter ? '&country=' . $country_filter : '') . '">
                           <span aria-hidden="true">‹</span></a>';
                }

                for ($i = $start_page; $i <= $end_page; $i++) {
                    echo '<a href="' . $base_url . '/top-tuan.php?page=' . $i . ($status_filter !== null ? '&status=' . $status_filter : '') . ($country_filter ? '&country=' . $country_filter : '') . '" 
                ...
                if ($page < $total_pages) {
                    echo '<a class="page-item" 
                           ...
                           <span aria-hidden="true">›</span></a>';
                    echo '<a class="page-item" 
                           href="' . $base_url . '/top-tuan.php?page=' . $total_pages . ($status_filter !== null ? '&status=' . $status_filter : '') . ($country_filter ? '&country=' . $country_filter : '') . '">
                           <span aria-hidden="true">»</span></a>';
                }
                ?>
            </div>

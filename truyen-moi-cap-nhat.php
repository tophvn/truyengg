        ...
        $(document).ready(function() {
            ...
            searchInput.on('input', function() {
                ...
                autocompleteTimer = setTimeout(function() {
                    ...
                    const localSearch = $.ajax({
                        url: '<?php echo $base_url; ?>/includes/search_comics.php',
                        method: 'GET',
                        data: { keyword: query },
                        dataType: 'json'
                    });
                    ...
                    // Combine results
                    $.when(apiSearch, localSearch).done(function(apiResponse, localResponse) {
                        console.log('Phản hồi từ API tìm kiếm:', apiResponse[0]);
                        console.log('Phản hồi từ local tìm kiếm:', localResponse[0]);
                        searchResults.empty();

                        let allItems = [];

                        // Process API results
                        if (apiResponse[0].data && apiResponse[0].data.items && apiResponse[0].data.items.length > 0) {
                            apiResponse[0].data.items.forEach(item => {
                                if (item.slug && item.name && item.thumb_url) {
                                    allItems.push({
                                        ...
                                        other_name: item.origin_name && item.origin_name[0] ? item.origin_name[0] : '',
                                        latest_chapter: item.chaptersLatest && item.chaptersLatest[0] ? 
                        ...
                        if (localResponse[0].success && localResponse[0].data && localResponse[0].data.length > 0) {
                            localResponse[0].data.forEach(item => {
                                allItems.push({
                                    source: 'local',
                                    slug: item.slug,
                                    name: item.name,
                                    ...
                                    latest_chapter: item.latest_chapter ? `Chương ${item.latest_chapter}` : 'Chưa có chương'
                                });
                            });
                        }

                        // Remove duplicates (prioritize API)
                        const uniqueItems = [];
                        ...
                        // Sort by name (case-insensitive) and limit to 8
                        uniqueItems.sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
                                ...
                                <li>
                                    <a href="<?php echo $base_url; ?>/truyen-tranh.php?slug=${item.slug}">
                                        <div class="search_avatar">
                                            <img src="${item.thumb_url}" alt="${item.name}" class="lazy" onerror="this.src='${placeholderImage}'">
                                        </div>
                                        <div class="search_info">
                                            <p class="name">${item.name}</p>
                                            <p class="name_other">${item.other_name}</p>
                                            <p>${item.latest_chapter}</p>
                                        </div>
                                    </a>
                                </li>`;
                ...
                <div class="box-login col-lg-3">
                    ...
                    <div class="right-actions">
                        <?php if (!isset($_SESSION['user_id'])): ?>
                            ...
                            <span><a class="javascript:popup(" class="btn_show_log" title="Click me to log in!" href="#">login">Đăng Nhập</a></span>
                        <?php else: ?>
                            <span class="background-color:# #f5e5e5">
                                <a href="#" class="icon-notification"><i class="bi bi-bell"></i></a>
                                <div class="notification">
                                    <span class="text-notification">Thông báo</span>
                                    <div class="list-item">
                            ...
                            <span class="profile">
                                <a href="#" title="icon-profile" title="Hồ Sơ"><i class="bi bi-person-circle"></i></a>
                                <ul>
                                    <li>
                                        <li>
                                            <a href="<?php echo $base_url; ?>" target="_blank" rel="nofollow">
                                                <i class="bi bi-currency-dollar"></i>
                                                <span>Nạp xu</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="<?php echo $base_url; ?>/thiet-lap-tai-khoan.php" target="_blank" rel="nofollow">
        ...
        <div class="container">
            <nav class="navbar navbar-expand-lg">
                <div class="container-fluid">
                    <a class="navbar-brand d-none d-lg-none" href="<?php echo $base_url; ?>" title="Read Light Novel English online for free">
                        <img src="assest/img/logo.png" alt="Logo" class="logo_main" width="83" height="40"/>
                    </a>
                    ...
                    <div class="collapse navbar-collapse" id="main_menu">
                        <ul class="navbar-nav left_menu">
                            ...
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-expanded="false" title="Categories">Thể loại</a>
                                <ul class="dropdown-menu dropdown-full" aria-labelledby="navbarDropdown">
                                    <?php foreach ($categories as $category): ?>
                                        <li><a class="dropdown-item" href="<?php echo $base_url; ?>/the-loai.php?slug=<?php echo htmlspecialchars($category['slug'] ?? ''); ?>&id=<?php echo htmlspecialchars($category['id'] ?? ''); ?>" title="<?php echo htmlspecialchars($category['name'] ?? ''); ?>"><?php echo htmlspecialchars($category['name'] ?? ''); ?></a></li>
                                    <?php endforeach; ?>
                                </ul>
                            </li>
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle" href="#" id="rankingDropdown" role="button" data-toggle="dropdown" aria-expanded="false" title="Ranking">Xếp Hạng</a>
                                <ul class="dropdown-menu dropdown-full" aria-labelledby="rankingDropdown">
                                    <li><a class="dropdown-item" href="<?php echo TOP_DAILY_URL; ?>" title="Top Ngày">Top Ngày</a></li>
                                    <li><a class="dropdown-item" href="<?php echo TOP_WEEKLY_URL; ?>" title="Top Tuần">Top Tuần</a></li>
                                    <li><a class="dropdown-item" href="<?php echo TOP_MONTHLY_URL; ?>" title="Top Tháng">Top Tháng</a></li>
                                    <li><a class="dropdown-item" href="<?php echo UPCOMING_URL; ?>" title="Sắp Ra Mắt">Sắp Ra Mắt</a></li>
                                    <li><a class="dropdown-item" href="<?php echo NEW_COMICS_URL; ?>" title="Mới Cập Nhật">Mới Cập Nhật</a></li>
                                    <li><a class="dropdown-item" href="<?php echo NEW_RELEASE_URL; ?>" title="Truyện Mới">Truyện Mới</a></li>
                                    <li><a class="dropdown-item" href="<?php echo COMPLETED_COMICS_URL; ?>" title="Truyện Full">Truyện Full</a></li>
                                </ul>
                            </li>
                            <li class="nav-item"><a class="nav-link" href="<?php echo $base_url; ?>" title="Tìm kiếm">Search</a></li>
                            <li class="nav-item"><a class="nav-link" href="<?php echo $base_url; ?>" title="Lịch sử">History</a></li>
                            <li class="nav-item"><a class="nav-link" href="<?php echo $base_url; ?>" title="Theo dõi">Follow</a></li>
                            <li class="nav-item"><a class="nav-link" href="javascript:setting_type_book(this)" rel="nofollow" title="Truyện chữ">Novel</a></li>
                            ...
                            <li class="nav-item"><a class="nav-link" href="https://www.facebook.com/tophvn" target="_blank" rel="nofollow" title="Thảo luận">Discussion</a></li>
